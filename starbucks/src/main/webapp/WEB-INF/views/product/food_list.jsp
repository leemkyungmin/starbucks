		...
		$().ready(function(){
			...
			$("input:checkbox[name='chkbox']").click(function(){ 
			 ...
			 $('.coffeeData a>img').on('click',function(){
				var fidx =$(this).parent().data('target');

				var form =document.createElement('form');
				form.setAttribute('method','post');
				form.setAttribute('action','${url}/menu/food/product_view');
				document.charset='utf-8';
				
				var data = document.createElement('input');
				data.setAttribute('type','hidden');
				data.setAttribute('name','fidx');
				data.setAttribute('value',fidx);
								...
								<dd class="${mtlist.getFtName2() }">
									<ul>
										...
										<c:forEach var="flist" items="${flist}">
											<c:if test="${flist.getFtIdx() eq mtlist.getFtIdx() }"  var="a">
												
												<li class="coffeeData">
													<dl>
														<dt>
															<a href="javascript:void(0)" data-target="${flist.getFIdx() }" class="coffee_img">
																<img alt="${flist.getFName()}"  src="${url}/resources/assets/images/food_img_test/${flist.getFImg()}">
															</a>
														</dt>
														<dd >${flist.getFName()}</dd>
													</dl>
												</li>
											</c:if>
										</c:forEach>
